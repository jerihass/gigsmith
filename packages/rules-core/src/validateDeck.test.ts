...
describe("validateDeck", () => {
  it("accepts a legal guide-format deck", () => {
    const result = validateDeck(createValidDeck(), cyberpunkCardDb, cyberpunkRulesetV1Printable);
  ...
  it("requires exactly 3 Legends", () => {
    ...
    const result = validateDeck(deck, cyberpunkCardDb, cyberpunkRulesetV1Printable);
  ...
  it("calculates cumulative RAM from selected Legends", () => {
    const report = calculateRamLimits(createValidDeck().legends, cyberpunkCardDb, cyberpunkRulesetV1Printable);
